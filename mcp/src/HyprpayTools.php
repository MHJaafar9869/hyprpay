...
final class HyprpayTools
{
    ...
    private const GATEWAY_GOTCHAS = [
        ...
        'cybersource_uc' => [
            ...
            [
                'title' => 'Void/refund the right RESOURCE — the payment path cannot reach an independent capture',
                'detail' => 'CyberSource scopes these by resource, not by payment. /pts/v2/payments/{id}/voids is documented as "only used if you requested Authorization and Capture TOGETHER"; once you capture separately the capture is its own resource with its own id and that path 404s. Use voidCapture()/refundCapture() (/pts/v2/captures/{id}/...) for a capture made via /payments/{id}/captures, voidRefund() (/pts/v2/refunds/{id}/voids) to cancel a refund before it settles, and voidCredit() (/pts/v2/credits/{id}/voids) for a standalone credit — a credit has no payment to reverse against, so this is the only way to undo one. Mapping: charge() with capture -> void(); charge(capture:false)+capture() -> voidCapture(); refund() -> voidRefund(); creditPayment() -> voidCredit().',
            ],
            [
                'title' => 'Timeout void/reversal needs merchantTransactionId on the ORIGINAL call — it cannot be added later',
                'detail' => 'When a payment, capture, refund, or credit times out you have no transaction id to void with, because the response that carried it never arrived. timeoutVoid() (POST /pts/v2/voids) and timeoutReversal() (POST /pts/v2/reversals) take no resource id at all — they match on clientReferenceInformation.transactionId from the original request. CyberSource: "make sure to pass unique value to field clientReferenceInformation -> transactionId in your payment, capture, refund, or credit API call and use same transactionId in this API request payload". So set ChargeRequest/CaptureRequest/RefundRequest/CreditRequest::$merchantTransactionId on anything you may need to undo blind; without it the timed-out request can only be reconciled, never reversed. This COMPLEMENTS findSuccessfulTransactionByReference(): that tells you WHETHER a lost request settled and is eventually consistent (seconds-to-minutes lag), while a timeout void UNDOES it and acts immediately. Without a timeout reversal a timed-out authorization strands a hold until the issuer expires it, which can be days.',
            ],
            [
                'title' => 'incrementAuthorization adds to the hold; a second authorization would double it',
                'detail' => 'PATCH /pts/v2/payments/{id} raises the amount already authorized, keeping ONE hold — for hotels, car rental, and any open-ended stay where the final bill is unknown when the card is presented. Authorizing again instead places a SECOND hold and withholds the funds twice from the cardholder. IncrementAuthorizationRequest::$additionalAmount is the amount to ADD, not the new total — passing the running total silently over-holds. Separately, creditPayment() (POST /pts/v2/credits) is NOT a refund: a refund returns part of a specific captured payment and is bounded by it, while a credit pushes money to a card with no originating transaction, no amount cap and nothing to tie it to. Processors watch credits; gate it behind payout-grade authorisation, not refund-grade, and pair it with voidCredit(). refreshPaymentStatus() re-checks with the PROCESSOR (for async APMs), unlike getTransaction() which reads CyberSource\'s own record — reach for it when the record looks stale rather than merely unfinished.',
            ],
            [
                'title' => 'BIN lookup before you charge — and one typed network from every source',
                'detail' => 'lookupBin() (POST /bin/v1/binlookup) answers what a card IS before authorizing: brand, funding source (CREDIT/DEBIT/PREPAID/...), issuer country, platform (consumer vs commercial), card product ("Visa Infinite"), and capability flags. That timing is the point — routing, surcharging, card-type offers, whether to offer installments, and whether 3DS is supported are all pre-charge decisions. Accepts a raw PAN or, preferably, a transient token (JWT or jti) or a vault customer/paymentInstrument/instrumentIdentifier reference, so no card data need leave the vault. CRITICAL: check BinLookupResult::isResolved() first — MULTIPLE and NO MATCH leave the attributes untrustworthy, and "unknown card" is NEVER grounds for refusing a payment; fall back to charging normally. Do NOT read the transient token\'s own claims for offer decisions: ParsesTransientToken decodes the JWT WITHOUT verifying its signature, so a shopper can edit it to claim an offer. The card brand previously reached the SDK three ways (code 001 from BIN lookup and the vault, lowercase "visa" from a verified orchestrated result, uppercase "VISA" from BIN lookup\'s brand field); CybersourceCardNetwork::resolve() now collapses all of them and BinLookupResult/PaymentInstrument/OrchestratedPaymentResult each expose network(), so branching on the network is one match. Unmodelled BIN attributes stay reachable verbatim via feature().',
            ],
            [
                'title' => 'Webhook onboarding: create the signing key FIRST, and it is shown only once',
                'detail' => 'createWebhookSecurityKey() (POST /kms/egress/v2/keys-sym) returns the value the webhook_secret credential holds — the same secret verifyWebhook() checks signatures against. Create it BEFORE the subscription: CyberSource signs every notification with it, so a subscription made first has nothing to sign with. The key is returned ONCE and cannot be read back; store it immediately or the subscription\'s notifications are unverifiable and the only fix is a new key. Then listWebhookProducts() to discover what the account is entitled to (product ids and event types are entitlement-scoped and events are nested PER product, which is why neither is an enum in this SDK — WebhookProductCatalog exposes eventNames() plus the per-event timeSensitivity and payloadEncryption flags), createWebhook(), then testWebhook() to prove the receiver before a real payment depends on it.',
            ],
            [
                'title' => 'Webhook security type decides whether you can verify anything; retries decide whether you lose events',
                'detail' => 'Only WebhookSecurityType::Key produces the signed notifications verifyWebhook() checks — the oAuth and oAuth_JWT variants make CyberSource fetch a bearer token from your authorization server instead, so those notifications are authenticated but NOT by this SDK and verifyWebhook() would fail closed on them. WebhookSubscription::isSignatureVerifiable() tells them apart; assert it at boot if your receiver assumes signatures. On retries, the algorithm is easy to misread: with firstRetry=10 and interval=30, ARITHMETIC retries at 10/40/70 minutes while GEOMETRIC retries at 10/300/9,000 — "later today" versus "in six days". deactivateOnFailure changes the failure shape entirely: on, an exhausted sequence SUSPENDS the subscription and queues everything until healthCheckUrl returns 200, then backfills (nothing lost, but you must have a health-check endpoint); off, each notification exhausts its own retries and is dropped while the subscription stays active. A silent integration is usually a suspended subscription rather than a gateway outage — check listWebhooks() and isDelivering() first. Prefer setWebhookStatus(INACTIVE) over deleteWebhook() when redeploying: delete is permanent and recreating yields a new id. updateWebhook() is a partial PATCH that deliberately cannot change status, so a routine URL edit can never silently reactivate a paused subscription.',
            ],
            [
