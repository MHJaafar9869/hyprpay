...
Non-interface extras: Paylink and Paytabs each expose `deleteToken(string): bool`; Cybersource exposes `confirmOrchestratedPayment(ConfirmOrchestratedPaymentRequest): OrchestratedPaymentResult` and `createMicroformSession(CheckoutSessionRequest): CheckoutSession` (Flex Microform v2 capture context).
...
**Operations:** `createCheckoutSession` (create order, return approval URL + order id), `charge` (complete approved order — capture if `request->capture`, else authorize), `capture` (`/v2/payments/authorizations/:id/capture`), `refund` (`/v2/payments/captures/:id/refund`), `void` and `reverseAuthorization` (both `/v2/payments/authorizations/:id/void` — PayPal has no distinct reversal), `vaultInstrument` (setup-token → payment-token), `chargeStoredCredential` (order with `payment_source.card.vault_id` + stored-credential metadata), `getTransaction`, `searchTransaction` / `listTransactions` / `listTransactionsByReference` / `findSuccessfulTransactionByReference` (Transaction Search / Reporting v1, `GET /v1/reporting/transactions`; a bare transaction id or a raw reporting query string, plus a default rolling window — 31 days max, `reporting_window_days` extra; reference lookups match `invoice_id` client-side because reporting cannot filter by merchant reference and lags real time by a few hours), `verifyWebhook`. Payer-auth, DCC, and wallet charges inherit-as-unsupported.

**Enums (`Enums/`):**
- `PayPalEndpoint` (`path(string $id)` substitutes `:id`): `OAuthToken` `/v1/oauth2/token`, `Orders` `/v2/checkout/orders`, `Order` `/v2/checkout/orders/:id`, `OrderCapture` `…/:id/capture`, `OrderAuthorize` `…/:id/authorize`, `AuthorizationCapture` `/v2/payments/authorizations/:id/capture`, `AuthorizationVoid` `/v2/payments/authorizations/:id/void`, `CaptureRefund` `/v2/payments/captures/:id/refund`, `SetupTokens` `/v3/vault/setup-tokens`, `PaymentTokens` `/v3/vault/payment-tokens`, `ReportingTransactions` `/v1/reporting/transactions`, `VerifyWebhookSignature` `/v1/notifications/verify-webhook-signature`.
- `PayPalReportingStatus` (Transaction Search `transaction_status` code) → `PaymentStatus`: `S` → Captured; `P` → Pending; `V` → Voided; `D` → Declined. `toPaymentStatusOrFailed(?string)` → Failed.
...
**Operations:** every interface op except `setupPayerAuth` (3DS device-data collection is folded into `INITIATE_AUTHENTICATION`, not a distinct step) — `createCheckoutSession`, `requestDccRate` (`POST /paymentOptionsInquiry`; `order.amount`/`currency` + `sourceOfFunds.provided.card.prefix` BIN → `currencyConversion` offer), `charge`, `chargeWallet` (Apple Pay / Google Pay, merchant-decrypted: `order.walletProvider` + `sourceOfFunds.provided.card.devicePayment`), `capture`, `refund`, `void`, `reverseAuthorization`, `enrollPayerAuth`, `validatePayerAuth`, `vaultInstrument`, `chargeStoredCredential`, `getTransaction`, `searchTransaction`, `findSuccessfulTransactionByReference` / `listTransactions` / `listTransactionsByReference` (RETRIEVE ORDER `GET /order/{id}` → per-transaction snapshots from `order.transaction[]`, newest first; the order id is the merchant reference), `verifyWebhook`.
...
- `MpgsEndpoint` (`path(array $params)` substitutes `{key}`, URL-encoded): `Session` `/session`, `SessionById` `/session/{sessionId}`, `Order` `/order/{orderId}`, `Transaction` `/order/{orderId}/transaction/{transactionId}`, `Token` `/token`, `TokenById` `/token/{tokenId}`, `PaymentOptionsInquiry` `/paymentOptionsInquiry` (DCC rate quote — a dedicated resource, not a transaction).
